...
app.get(
  ...
  async function (req, res, next) {
    ...
    const firstName = order.customer
      ? JSON.parse(order.customer).first_name ?? null
      : null;
    const profileCharcteristics = order.profile
      ? JSON.parse(order.profile)
      : [];
    res.render("admin/Letters/Profile_Letter", {
      firstName,
      profileCharcteristics,
      get_page_name: () => "Letter",
      _base_url: "/admin/letters/profile",
    });
  }
);

app.get(
  "/admin/letters/formula",
  SessionService.verifySessionMiddleware(role, "admin"),
  async function (req, res, next) {
    const { order: orderId } = req.query;
    ...
    if (activesNames && activesNames.length)
      activesNames = activesNames.filter((active) => active !== "Base");
    const { rows: actives, count } = await db.active.findAndCountAll({
      where: { name: { [db.Sequelize.Op.in]: activesNames } },
    });

    res.render("admin/Letters/Formula_Letter", {
      actives,
      count,
      get_page_name: () => "Letter",
      _base_url: "/admin/letters/formula",
    });
  }
);

app.get(
  "/admin/letters/instructions",
  SessionService.verifySessionMiddleware(role, "admin"),
  async function (req, res, next) {
    const { order: orderId } = req.query;
    ...
    res.render("admin/Letters/Instructions_Letter", {
      get_page_name: () => "Letter",
      _base_url: "/admin/letters/instructions",
    });
  }
);
...
app.get(
  ...
  async function (req, res, next) {
    ...
    res.render("admin/Letters/All", {
      firstName,
      profileCharcteristics,
      actives,
      count,
      get_page_name: () => "Letter",
      _base_url: "/admin/letters/all",
    });
  }
);
...
app.get("/configurations", async function (req, res, next) {
  try {
    ...
    if (!config) {
      return res
        .status(404)
        .json({ success: false, message: "No configurations found" });
    }
...
app.get("/count", async function (req, res, next) {
  try {
    // const a = await db;
    // console.log("dfsfsfsfsfsfsfsdfsd", a);
    const config = await db.counter.findOne({
      order: [["id", "DESC"]],
    });
    if (!config) {
      return res
    ...
    console.log("adadadadadadadadadadadad", config.dataValues);
    const resp = config.dataValues.text
      ? {
          text: config.dataValues.text,
          time: config.dataValues.time,
        }
      : {};
